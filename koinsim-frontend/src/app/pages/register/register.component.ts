...
@Component({
    ...
    <div class="auth-bg">
      <div class="auth-card">
        <div class="auth-header">
          <div class="logo">₿</div>
          <h1>Crea Account</h1>
          <p>Inizia a simulare il tuo portafoglio</p>
        </div>

        <form [formGroup]="form" (ngSubmit)="submit()" class="auth-form">
          <mat-form-field appearance="outline">
            <mat-label>Nome utente</mat-label>
            <input matInput formControlName="nomeUtente" placeholder="Scegli un username">
            <mat-icon matPrefix>person</mat-icon>
            <mat-error *ngIf="form.get('nomeUtente')?.hasError('required')">Campo obbligatorio</mat-error>
            <mat-error *ngIf="form.get('nomeUtente')?.hasError('minlength')">Minimo 3 caratteri</mat-error>
          </mat-form-field>

          <mat-form-field appearance="outline">
            <mat-label>Email</mat-label>
            <input matInput formControlName="email" type="email" placeholder="La tua email">
          ...
          <mat-form-field appearance="outline">
            ...
            <mat-error *ngIf="form.get('password')?.hasError('minlength')">Minimo 6 caratteri</mat-error>
          </mat-form-field>
  ...
  styles: [`
    ...
    h1 { font-size: 26px; font-weight: 700; color: var(--text-primary); letter-spacing: -0.5px; }
    p { color: var(--text-secondary); font-size: 13px; margin-top: 6px; }
...
export class RegisterComponent {
  ...
  form = this.fb.group({
    nomeUtente: ['', [Validators.required, Validators.minLength(3)]],
  ...
  submit(): void {
    ...
    this.auth.register(this.form.value as any).subscribe({
      next: () => {
        this.snack.open('Account creato! Accedi ora.', 'OK', { duration: 3000, panelClass: 'snack-success' });
        this.router.navigate(['/login']);
      },
      error: (err) => {
        this.loading = false;
        const msg = err.status === 409 ? 'Username o email già in uso' : 'Errore durante la registrazione';
